...
Orm.prototype.joinUser = function (opts) {
  var opts = opts || {};
  let select = opts.select ? opts.select : 'u.sex AS sex,u.username AS user_name,u.avatar_path AS avatar_path';
  let foreignKey = opts.foreignKey ? opts.foreignKey : 'user_id';
  this.addSelect(select);
  this.join('LEFT JOIN sc_user u ON u.id=' + this._alias + '.' + foreignKey);
  opts = null;
  return this;
};
...
Orm.prototype.setAttribute = function (key, value) {
  if (this._attribute === null || this._attribute.hasOwnProperty(key)) {
    ...
    if (key.indexOf('is_') >= 0) {
      ...
      if (validator.isDate(value) && !validator.isTime(value)) {
        value = value + ' 00:00:00'
      } else if (!validator.isTime(value)) {
        value = '0000-00-00 00:00:00'
      }
    } else if (validator.isIn(key, ['contact_id'])) {
